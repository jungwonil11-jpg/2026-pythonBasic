...
강사 GitHub URL: https://docs.python.org/3/library/index.html
...
### 4. upstream fetch
upstream 등록 여부 확인: `git remote -v`
- upstream 없으면 먼저 등록:
  ```
  git remote add upstream https://docs.python.org/3/library/index.html
  ```
- fetch 실행:
  ```
  git fetch upstream
...
### 12. 완료 요약
다음 항목을 출력:
- 변경된 파일 목록 (`git diff --name-only main@{1} main` 등으로 확인)
- 충돌 발생 여부 및 해결 방법
